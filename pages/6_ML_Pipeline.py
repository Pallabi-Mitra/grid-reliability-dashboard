# ============================================================
# PAGE: MACHINE LEARNING PIPELINE
...
def feature_engineer(state: PipelineState) -> PipelineState:
    ...
    if "season" not in merged.columns and "month" in merged.columns:
        def get_season(mo):
            ...
            elif mo in [3, 4, 5]: return "Spring"
            elif mo in [6, 7, 8]: return "Summer"
            else: return "Fall"
    ...
    return {**state, "engineered_df": merged, "X": X}

def model_runner(state: PipelineState) -> PipelineState:
    if not state["validation_passed"]:
        return state
    X = state["X"]
    eng_df = state["engineered_df"].copy().reset_index(drop=True)
    ...
    preds_df = eng_df.copy()
    if best in results and results[best]["preds"] is not None:
        preds_df["predicted_impact_ratio"] = results[best]["preds"]
