...
latest_df["predicted_impact_ratio"] = _booster.predict(
...
def get_shap_explanation(asset_id: str) -> str:
    """
    Get top 5 risk drivers for a specific generator asset ID like AST-023.
    Returns feature importance scores using XGBoost gain-based importance.
    """
    ...
    lines = [
        f"Risk drivers for {asset_id} ({asset_info['fuel_category']}, Zone {asset_info['operating_region']}):",
        ...
        f"\nTop 5 risk drivers (by feature importance):"
    ]
    for feature, importance in top5.items():
        lines.append(f"  - {feature.replace('_', ' ')}: importance {importance:.4f}")
    return "\n".join(lines)
